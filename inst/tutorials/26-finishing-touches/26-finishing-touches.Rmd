...
predictions <- data.frame(
  category = c("Early Spring", "Longer Winter"),
  count = c(55, 19)
)

rows <- 5

waffle_predictions <- predictions |>
  uncount(count) |>
  mutate(
...
We can add labels and themes, but you have to be careful. For this to look like a waffle chart, we have to use `theme_void()`. Here is an example of how to use ggplot's `labs` and the `theme` to make a fully publication ready graphic. They just get added with a + after the waffle.

```{r waffle1, exercise=TRUE, exercise.setup = "waffle-load-data", message=FALSE}
ggplot() +
  geom_tile(
    data=waffle_predictions, 
    aes(
...
The same additions work for the multiple chart version from the waffle chapter. 
