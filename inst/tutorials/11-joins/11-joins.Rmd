...
In 2002, I used FARS data to write about a stretch of highway in Pasco County, Florida that is one of the nations most deadly places to walk across the street. I looked at deaths from 1990-2000 and wrote this section of that story:
...
The hard way, in spite of it's name, isn't that hard. We just have to tell the `inner_join` what column from our left data (accidents) is going to match what column from our right data (people). So we're going to put accidents on the left -- that would be the first position -- and people on the right. Remember: the common column between them that we'll use is ST_CASE. *Both* side have this one column. 
