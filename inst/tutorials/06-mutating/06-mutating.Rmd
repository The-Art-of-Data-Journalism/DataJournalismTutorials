...
first <- counties |> mutate(
  percent_change = ((POPESTIMATE2025 - POPESTIMATE2025)/POPESTIMATE2025)*100
) |> slice(1)

fastest <- counties |> mutate(
  percent_change = ((POPESTIMATE2025 - POPESTIMATE2024)/POPESTIMATE2024)*100
) |>
  arrange(desc(percent_change)) |> slice(1)

biggest <- counties |> mutate(
  rawchange = POPESTIMATE2025 - POPESTIMATE2024
) |>
...
If we look at what we got when we imported the data, you'll see there's a column called POPESTIMATE2025 as the new data, and we'll use POPESTIMATE2024 as the old data. So we're looking at one year. Then, to help us, we'll use arrange again to sort it, so we get the fastest growing county over one year. 
...
counties |> mutate(
  percent_change = (POPESTIMATE2025 - POPESTIMATE2024)/POPESTIMATE2024
) 
```
