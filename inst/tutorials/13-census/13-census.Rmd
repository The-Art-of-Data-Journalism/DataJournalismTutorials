...
estimates25 <- get_estimates(geography="county", vintage = 2025, state=state)
estimates24 <- get_estimates(geography="county", vintage = 2024, state=state)

nrows <- nrow(estimates25)
...
popest <- bind_rows(pop25, pop24)

acs <- load_variables(2024, "acs5")
...
The first thing you must do is get an API key from the Census Bureau. Previous students of this class didn't have to do this. You do now the first time you run this. So how do you get an API key? 

1. Go to [this website](https://api.census.gov/data/key_signup.html) and fill out the request for a key. 
2. Your key will arrive in your email. Copy the long hash of letters and numbers. 
3. Run this in your console: `tidycensus::census_api_key("PASTE YOUR KEY HERE", install = TRUE)`
4. Then quit R Studio and restart this tutorial.

Once you've done that, it's all set up, you don't need to do this again. 

The `tidycensus` library makes calls to the Census API in a very tidy way, and gives you back tidy data. That means we don't have to go through the process of importing the data from a file, cleaning it up and making it a dataframe. I can't tell you how amazing this is, speaking from experience. My first experience with the census was for the 2000 data. I was part of a team at the Tampa Bay Times that had to pull the data in, chop it up for reporters working on specific stories and send that data out to them and graphic artists to make charts and maps out of it. What we're going to do today took **hours** on that day in 2001 when the data officially dropped. Just getting the data took hours. You're going to do it in seconds. 
...
Behind the scenes, I've created estimates24 for you, which is identical to estimates25 except it's ... a different year. The problem now is we have two dataframes instead of one, and way too much data. Let's look at our data first so we can see the problem before we fix it. Let's just run head on the 2025 estimates dataframe. 
...
Now we have two dataframes of three columns: `pop25`, and `pop24`. We could join them using the county name, but the problem then is we have two different years and the column names for the estimate are the same. We won't know which year is which without some careful connecting-the-dots in our code. 
...
Our next problem? We have data that's stacked, not side by side. To calculate the percent change in population, you need the 2025 population - the 2024 population / the 2024 population -- new - old / old. To do that, they have to be **columns**. You have **rows**. So how do we flip our data on its side, so to speak, and turn each row for a county into two columns for a county? 
...
Let's use the 5-year ACS from 2024, the most recent year 5-year ACS data is available for, as our example here. `tidycensus` shortens that to "acs5

```{r variable-data, exercise=TRUE, exercise.setup = "load-data"}
acs <- load_variables(2024, "____")
```
