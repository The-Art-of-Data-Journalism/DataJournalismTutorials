...
We can do that with some clever manipulations of the x axis. In ggplot, there are all kinds of ways to work with the x and y axis scales. In this case, we have a year on the x axis, which to R is just a number, so we'll use `scale_x_continuous` to work with that kind of scale. We're going to first set the line breaks manually so there's just 2020 and 2026. Then, we're going to set the limits of our x axis so that it doesn't just default to 2020-2026, but is a little wider so we have extra space. We're going to add one year on each side to pad it out. But think about it -- how do you *add* one year of space to the *left* of 2020? You subtract one year. How do you *add* a year of space to the *right* of 2026? You add one year. 
...
ggplot() + 
  ...
  geom_line(data=state_peak, aes(x=year, y=Unemployment_Rate, group=Location), color="red") +
  geom_point(data=state_peak, aes(x=year, y=Unemployment_Rate, group=Location), color="red") + 
  scale_x_continuous(breaks = c(2020, 2026), limits = c(2019, 2027)) 
```
```{r slope4-check}
grade_this_code()
```
...
ggplot() + 
  ...
  scale_x_continuous(breaks = c(2020, 2026), limits = c(2019, 2027.5)) 
```
